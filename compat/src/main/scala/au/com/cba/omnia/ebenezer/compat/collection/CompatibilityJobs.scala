...
package au.com.cba.omnia.ebenezer.compat.collection
...
import au.com.cba.omnia.ebenezer.compat.Collection
...
object Compatibility {
  ...
  val dataTupled = data.map(t => (t._1, t._2, t._3, t._4, t._5, t._6, t._7, t._8, t._9, t._10, t._11))
  ...
  def schema(db: String, table: String) = s"""
    CREATE TABLE $db.$table (
      ...
      listfield array<string>, mapfield map<string, int>
...
class MRWriteJob(args: Args) extends Job(args) {
  ...
  IterablePipe(Compatibility.data, flowDef, mode)
    .write(HiveParquetScroogeSource[Collection](args("db"), args("dst"), location))
}

class MRReadJob(args: Args) extends Job(args) {
  HiveParquetScroogeSource[Collection](args("db"), args("src"))
...
class HiveReadJob(args: Args) extends CascadeJob(args) {
  ...
  val table = HiveParquetScroogeSource[Collection](db, dst)
...
class HiveWriteJob(args: Args) extends CascadeJob(args) {
  ...
  val srcTable = HiveParquetScroogeSource[Collection](db, src)
  val table    = HiveParquetScroogeSource[Collection](db, dst)
