...
class PartitionScroogeWriteSupport[A <: ThriftStruct] extends WriteSupport[A] {
  ...
  def init(config: Configuration): WriteContext = {
    val partitions = Option(config.getStrings("ebenezer.partition_fields")).getOrElse(Array())
    val thrift = ScroogeReadWriteSupport.getThriftClass[A](config)
    val converter = new ScroogeStructConverter(partitions.toSet)
    ...
    if (partitions.isEmpty) extra.put("thrift.class", thrift.getName)
    else                    extra.put("thrift.class", "partitioned_" + thrift.getName)
